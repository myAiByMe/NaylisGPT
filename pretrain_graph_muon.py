#!/usr/bin/env python3
"""
pretrain_graph_muon.py — GNT graph_scale dans Muon
====================================================
Variante expérimentale : graph_scale mis directement dans l'optimiseur Muon.

  - use_graph  = True  (hard-codé)
  - graph_scale → groupe Muon (LR ×5 + momentum SGD sans Newton-Schulz car 1D)

NOTE technique :
  Newton-Schulz requiert ndim >= 2. Pour graph_scale (1D, shape [num_heads]),
  Muon applique un momentum SGD simple au LR Muon — pas de whitening, mais le
  LR ×5 et le momentum 0.95 s'appliquent. C'est la vraie intégration Muon,
  pas juste un LR×5 via AdamW.

USAGE :
  python pretrain_graph_muon.py
  python pretrain_graph_muon.py --no-compile
  python pretrain_graph_muon.py --total-steps 45732
"""
...
def get_args():
    ...
    p.add_argument('--total-steps',  type=int, default=None,
                   help='Forcer total_steps du scheduler WSD pour reprise.')
    return p.parse_args()
...
CONFIG = {
    ...
    'use_flash_attn'        : True,
    'rel_rank'              : 32,
    'use_graph'             : True,   # hard-codé — ce script est le run Muon graph
    ...
    'checkpoint_file'       : './Model/naylis_gnt_muon.pt',
    # Compile
...
print('=' * 70)
print('  GNT-Naylis — Pretrain | graph_scale → Muon (momentum SGD 1D)')
print('=' * 70)
if DEVICE == 'cuda':
...
print(f'  batch_size={CONFIG["batch_size"]}  gradient_acc={CONFIG["gradient_accumulation"]}  '
      f'use_graph=True  graph_scale=Muon')
...
class Muon(torch.optim.Optimizer):
    """
    Muon + MARS-M pour matrices 2D.
    Gestion 1D (graph_scale) : momentum SGD sans Newton-Schulz.
      - Newton-Schulz requiert ndim >= 2 → inapplicable sur [num_heads]
      - Le 1D reçoit quand même le LR Muon ×5 et le momentum 0.95
      - Même MARS-M appliqué pour la cohérence du momentum
    """
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if g.ndim < 2:
                    # ── 1D (graph_scale) : momentum SGD au LR Muon ──────────
                    # Pas de Newton-Schulz (requiert matrice), mais :
                    #   - même momentum 0.95 que Muon
                    #   - MARS-M appliqué pour cohérence du gradient
                    #   - LR ×5 via le scheduler (is_muon=True sur le groupe)
                    if use_mars:
                    ...
                    buf = state['buf']
                    buf.mul_(mom).add_(g)
                    g_step = (g + mom * buf) if nest else buf
                    if wd: p.mul_(1. - lr * wd)
                    p.add_(g_step, alpha=-lr)
                    continue

                # ── 2D+ : MARS-M + Newton-Schulz (comportement original) ────
                if use_mars:
                    if 'prev_grad' not in state:
                        state['prev_grad'] = torch.zeros_like(g)
                    prev = state['prev_grad']
...
def configure_optimizers(model, lr: float, weight_decay: float, betas, eps):
    """
    Répartition :
      EXCLUDE (embeddings, head)  → AdamW decay/nodecay
      blocks.* dim >= 2           → Muon  (matrices, Newton-Schulz)
      blocks.* graph_scale (1D)   → Muon  (momentum SGD, LR ×5)
      blocks.* autres 1D          → AdamW nodecay
      reste dim >= 2              → AdamW decay
      reste 1D                    → AdamW nodecay
    """
    ...
    for pn, p in model.named_parameters():
        ...
        if pn in EXCLUDE:
            ...
            if 'graph_scale' in pn:
                muon_params.append(p)   # ← graph_scale va dans Muon
            else:
    ...
    n_gs    = sum(p.numel() for pn, p in model.named_parameters() if 'graph_scale' in pn)

    print(f'\n  Muon+MARS  : {n_muon / 1e6:.3f}M params  lr={lr * 5.0:.2e}  (base×5 via scheduler)')
    print(f'               dont graph_scale : {n_gs} scalaires (momentum SGD 1D, pas de NS)')
...
def train_one_chunk(
    ...
    train_ds = chunk.train_dataset(CONFIG['max_seq_len'], CONFIG['use_packing'], EOS_ID)
    ...
    if is_resume and batches_done_in_chunk > 0:
        print(f'  ↩️  Reprise : {batches_done_in_chunk} batches  ({steps_done_in_chunk} steps), '
              f'{len(indices)} samples restants')
    ...
    collate = (partial(packed_collate_fn, eos_token_id=EOS_ID, seq_len=CONFIG['max_seq_len'])
        ...
        try:
            ...
            if acc_steps >= CONFIG['gradient_accumulation']:
                ...
                if global_step % CONFIG['validate_every_steps'] == 0:
                    ...
                    pbar.write(f'  [val  step={global_step:,}] loss={vloss:.4f}  ppl={ppl:.2f}')
...
def main():
    ...
    if ARGS.total_steps is not None:
        print(f'\n  ⚠️  --total-steps forcé : {sched_total_steps:,} (calculé={TOTAL_STEPS:,})')
    ...
    if cp:
        ...
        print(f'  Resume warmup : 100 steps')

        if current_chunk_idx >= len(ALL_CHUNKS):
            print('\n✅ Training déjà terminé.')
            return

    print('\n' + '=' * 70)
    print(f'  TRAINING START — {TOTAL_STEPS:,} steps — {len(ALL_CHUNKS)} chunks')
    print(f'  graph_scale → Muon (momentum SGD 1D, LR ×5)')
    print('=' * 70)

    for chunk_idx, chunk_info in enumerate(ALL_CHUNKS):
        if chunk_idx < current_chunk_idx:
            print(f'  ⏩ chunk_{chunk_info["id"]:03d} — déjà traité, skip')
            continue
        ...
        try:
            ...
            print('  ✅ OK — relancer pour reprendre')
            return
    ...
    print(f'  Steps : {global_step:,}  |  Temps : {total_time / 3600:.2f}h  |  Chunks : {len(ALL_CHUNKS)}')
    print(f'{"="*70}')
