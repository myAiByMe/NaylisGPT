...
class NaylisAttention(nn.Module):
    ...
    def __init__(
        ...
        # Si use_graph=False, on désalloue les projecteurs relationnels pour économiser la mémoire
        if not use_graph:
            del self.rel_q_proj, self.rel_k_proj, self.graph_scale
    ...
    def forward(
        ...
        # graph bias — uniquement en prefill (S>1) et si use_graph=True
        # varlen (flash_attn_varlen) ne supporte pas attn_mask dense → désactivé
        use_varlen = (
            cu_seqlens_q is not None
            and self._fa_varlen is not None
            and past_kv is None
        )
        _use_graph = self.use_graph and (not use_varlen) and (S > 1 or past_kv is None)
        graph_bias = self._compute_graph_bias(x, q.dtype) if _use_graph else None
        ...
        if use_varlen:
            ...
            if is_causal and graph_bias is not None:
                # graph_bias + masque causal fusionnés dans attn_mask
                ...
                output = F.scaled_dot_product_attention(
                    q, k, v,
                    attn_mask = graph_bias.contiguous(),
                    is_causal = False,
                    dropout_p = self.dropout.p if self.training else 0.0,
                    scale     = scale,
                )
        ...
        elif self._fa_func is not None and mask is None:
            is_causal = (S > 1 and past_kv is None)
